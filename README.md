...
The project contains a custom game engine, a structured data model and a JavaFX graphical user interface.  
The code follows an MVC-style structure by separating controllers, model classes, services and resources.
...
- `BattleController`
- FXML views
- JavaFX properties and observable lists

### I4

The application uses multithreading and asynchronous processing to keep the GUI responsive.
...
- `ExecutorService`
- `CompletableFuture`
- background TCP server threads
- `Platform.runLater` for safe JavaFX UI updates

### I5

The application supports persistent game state storage using Java serialization and restores the previous state through deserialization.
...
The project also includes a Reflection-based documentation generator.
...
The project implements network communication for a multiplayer environment.

Implemented examples:

- TCP socket communication for battle moves
- threaded socket server
- RMI service for lobby registration and chat
- JNDI lookup for remote service access
...
- `RmiLobbyServer`
- `RmiLobbyClient`
- `GameLobbyRemoteService`
- `GameLobbyRemoteServiceImpl`

### I7

The application supports dynamic XML configuration.
